...
namespace VoiceLuisApp.Listeners
{
    public class CameraCaptureListener : CameraCaptureSession.CaptureCallback
    {
        ...
        private void Process(CaptureResult result)
        {
            switch (owner.mState)
            {
                case CameraBasic.STATE_WAITING_LOCK:
                    {
                                   ...
                                   (((int)ControlAFState.NotFocusedLocked) == afState.IntValue()))
                        {
                            ...
                            {
                                owner.mState = CameraBasic.STATE_PICTURE_TAKEN;
                                owner.CaptureStillPicture();
                            }
                            else
                            {
                ...
                case CameraBasic.STATE_WAITING_PRECAPTURE:
                    {
                        // ControlAeState can be null on some devices
                        Integer aeState = (Integer)result.Get(CaptureResult.ControlAeState);
                        if (aeState == null ||
                        ...
                        {
                            owner.mState = CameraBasic.STATE_WAITING_NON_PRECAPTURE;
                        }
                        break;
                    }
                case CameraBasic.STATE_WAITING_NON_PRECAPTURE:
                    {
                        // ControlAeState can be null on some devices
                        Integer aeState = (Integer)result.Get(CaptureResult.ControlAeState);
